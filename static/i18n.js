...
function trackLanguagePreference(lang, source) {
  ...
  window.umami.track('language_preference', {language: lang, source: source});
}
...
function setSwitchButtonState(button, lang) {
  ...
  button.setAttribute(
      'aria-label',
      lang === 'uk' ? 'Switch language to English' :
...
function applyTranslations(translations, lang, source) {
  ...
  document.querySelectorAll('[data-i18n-attr]').forEach((node) => {
    ...
    const pairs =
        descriptor.split(';').map((item) => item.trim()).filter(Boolean);
    pairs.forEach((pair) => {
      const [attrName, key] =
          pair.split(':').map((part) => part && part.trim());
